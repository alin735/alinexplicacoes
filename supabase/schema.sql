...
CREATE TABLE IF NOT EXISTS group_classes_waitlist (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE UNIQUE INDEX IF NOT EXISTS group_classes_waitlist_email_unique_idx
  ON group_classes_waitlist (LOWER(email));

DROP TRIGGER IF EXISTS set_group_classes_waitlist_updated_at ON group_classes_waitlist;
CREATE TRIGGER set_group_classes_waitlist_updated_at
  BEFORE UPDATE ON group_classes_waitlist
  FOR EACH ROW
  EXECUTE FUNCTION public.set_updated_at();

ALTER TABLE group_classes_waitlist ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Admin can view group classes waitlist" ON group_classes_waitlist;
DROP POLICY IF EXISTS "Service role manages group classes waitlist" ON group_classes_waitlist;

CREATE POLICY "Admin can view group classes waitlist" ON group_classes_waitlist
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND is_admin = TRUE)
  );

CREATE POLICY "Service role manages group classes waitlist" ON group_classes_waitlist
  FOR ALL USING (auth.role() = 'service_role')
  WITH CHECK (auth.role() = 'service_role');
