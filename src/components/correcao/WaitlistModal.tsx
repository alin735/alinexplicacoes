...
type Props = {
  ...
  onJoined: () => void;
  // Apenas no modo "funnel": seguir em frente sem entrar na lista.
  onSkip?: () => void;
  // Origem registada na lista de espera (ex.: "secundario"). Sem valor, usa a predefinição da API.
  source?: string;
  // Texto do botão principal no modo "funnel" (ex.: "Ver o meu resultado →").
  funnelLabel?: string;
};
...
export default function WaitlistModal({
  ...
  onSkip,
  source,
  funnelLabel = 'Seguir para a correção →',
}: Props) {
        ...
        <div className="mt-5 space-y-3">
          ...
          {mode === 'funnel' && (
            <p className="text-center text-xs text-gray-400">
              Para seres avisado quando abrirmos as Explicações Top, assinala a opção acima. Caso
              contrário, basta clicares no botão para continuar.
            </p>
          )}
        </div>
      </div>
