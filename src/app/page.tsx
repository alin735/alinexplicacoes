        ...
        <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-[#0d2f4a] via-[#1a5276] to-[#2980b9]">
          ...
          <div className="absolute bottom-0 left-0 right-0">
            <svg viewBox="0 0 1440 120" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
                d="M0 120L48 110C96 100 192 80 288 70C384 60 480 60 576 65C672 70 768 80 864 85C960 90 1056 90 1152 80C1248 70 1344 50 1392 40L1440 30V120H0Z"
                fill="#f0f4f8"
              />
            </svg>
          </div>
        ...
        <section className="py-20 px-4 bg-gradient-to-br from-[#0d2f4a] via-[#1a5276] to-[#2980b9]">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[
                { name: 'Matemática', emoji: '📐', color: 'from-blue-400 to-blue-600' },
                { name: 'Físico-Química', emoji: '⚗️', color: 'from-purple-400 to-purple-600' },
                { name: 'Biologia-Geologia', emoji: '🧬', color: 'from-green-400 to-green-600' },
                { name: 'Português', emoji: '📖', color: 'from-amber-400 to-amber-600' },
              ].map((subj, i) => (
                <Link
                  key={i}
                  href="/marcar"
                  className="bg-white/10 backdrop-blur-sm border border-white/10 rounded-2xl p-6 text-center hover:bg-white/20 hover:-translate-y-2 hover:border-white/30 transition-all duration-300 group"
                >
                  <div className="text-4xl mb-4 group-hover:scale-125 transition-transform duration-300">
                    {subj.emoji}
                  </div>
                  <h3 className="text-white font-semibold text-sm sm:text-base">{subj.name}</h3>
                </Link>
              ))}
            </div>
          </div>
